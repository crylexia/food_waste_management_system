...
class AnalyticsService
{
    ...
    public function getMeaningfulInsights(): array
    {
        ...
        $totalUsedValue = $comparison->sum('used_value');
        ...
        if ($mostWasted) {
            $insights[] = [
                ...
                'message' => "{$mostWasted->name} contributes the highest total waste with " . number_format($mostWasted->total_wasted, 2) . " units, worth ₱" . number_format($mostWasted->wasted_value, 2) . ".",
            ];
        }
        ...
        if ($mostUsed) {
            $insights[] = [
                ...
                'message' => "{$mostUsed->name} is the most used item with " . number_format($mostUsed->total_used, 2) . " units consumed, worth ₱" . number_format($mostUsed->used_value, 2) . ".",
            ];
        }
        ...
        if ($highestWasteRate) {
            $insights[] = [
                ...
                'message' => "{$highestWasteRate->name} has the highest waste rate at " . number_format($highestWasteRate->waste_rating, 1) . "%.",
            ];
        }
        ...
        if ($highestWasteValue) {
            $insights[] = [
                ...
                'message' => "{$highestWasteValue->name} caused the highest monetary loss at ₱" . number_format($highestWasteValue->wasted_value, 2) . ".",
            ];
        }

        if (($totalUsed + $totalWasted) > 0) {
            if ($overallWasteRate < 10) {
                $healthMessage = "Overall inventory performance is excellent with a low waste rate of " . number_format($overallWasteRate, 1) . "%. Total waste cost is ₱" . number_format($totalWastedValue, 2) . ".";
                $healthType = 'success';
                $healthIcon = '✅';
            } elseif ($overallWasteRate <= 20) {
                $healthMessage = "Overall inventory performance is acceptable with a moderate waste rate of " . number_format($overallWasteRate, 1) . "%. Total waste cost is ₱" . number_format($totalWastedValue, 2) . ".";
                $healthType = 'info';
                $healthIcon = '📊';
            } else {
                $healthMessage = "Overall inventory waste is high at " . number_format($overallWasteRate, 1) . "%. Total waste cost is ₱" . number_format($totalWastedValue, 2) . ".";
                $healthType = 'danger';
                $healthIcon = '🚨';
            }
        ...
        if ($highestWasteValue && $highestWasteValue->wasted_value > 0) {
            $insights[] = [
                ...
                'message' => "Review purchasing and storage for {$highestWasteValue->name} first because it currently creates the highest financial loss at ₱" . number_format($highestWasteValue->wasted_value, 2) . ".",
            ];
        }
