...
{{-- ── ROW 3: Expiry Alerts (from Storage) ─────────────────────── --}}
@if($expiryAlerts->isNotEmpty())
<div class="section-block">
    <div class="section-header">
        <span>⏰ Expiry Alerts</span>
        <a href="{{ route('storage.index') }}" class="see-more">View storage →</a>
    </div>
    <div class="alert-list">
        @foreach($expiryAlerts as $item)
            <div class="alert-item alert-{{ $item->expiry_status === 'expired' ? 'danger' : ($item->expiry_status === 'critical' ? 'danger' : 'warning') }}">
                <span class="alert-icon">
                    {{ $item->expiry_status === 'expired' ? '🔴' : ($item->expiry_status === 'critical' ? '🟠' : '🟡') }}
                </span>
                <div class="alert-body">
                    <div class="alert-title">{{ $item->name }}</div>
                    <div class="alert-msg">
                        @if($item->expiry_status === 'expired')
                            Expired on {{ $item->expiration_date->format('M j, Y') }} — {{ $item->quantity }} {{ $item->unit ?? 'units' }} remaining
                        @else
                            Expires {{ $item->expiration_date->format('M j, Y') }}
                            ({{ $item->expiration_date->diffForHumans() }}) — {{ $item->quantity }} {{ $item->unit ?? 'units' }} remaining
                        @endif
                    </div>
                </div>
            </div>
...
{{-- ── ROW 4: Top 3 Wasted + Quick Actions ────────────────────── --}}
<div class="two-col">
    ...
    <div class="section-block">
        ...
        <div class="action-list">
            ...
            <a href="{{ route('storage.index') }}" class="action-card {{ $expiryAlerts->isNotEmpty() ? 'warn' : '' }}">
                <span class="action-icon">🧊</span>
                <div>
                    <div class="action-label">Manage Storage</div>
                    <div class="action-sub">
                        @if($expiryAlerts->isNotEmpty())
                            {{ $expiryAlerts->count() }} item{{ $expiryAlerts->count() > 1 ? 's' : '' }} need attention
                        @else
                            Track inventory &amp; expiry dates
                        @endif
                    </div>
                </div>
            </a>
...
{{-- ── ROW 5: Use First (expiring within 7 days from Storage) ──── --}}
@if($useFirst->isNotEmpty())
<div class="section-block">
    <div class="section-header">
        <span>🥗 Use These First</span>
        <a href="{{ route('storage.index') }}" class="see-more">View all storage →</a>
    </div>
    <ul class="use-first-list">
        @foreach($useFirst as $item)
            <li class="use-first-item">
                <span class="use-first-name">{{ $item->item->name }}</span>
                <span class="use-first-qty">{{ $item->quantity }} {{ $item->unit ?? 'units' }}</span>
                <span class="use-first-date expiry-{{ $item->expiry_status }}">
                    Exp. {{ $item->expiration_date->format('M j') }}
                    @if($item->expiration_date->isToday())
                        <strong>(today!)</strong>
                    @elseif($item->expiration_date->isTomorrow())
...
.action-card.warn    { border-color: #F9A825; background: #FFFDE7; }
...
/* ── Use First list ── */
.use-first-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0; }
.use-first-item {
    display: flex;
    align-items: center;
    gap: 12px;
...
.use-first-item:last-child { border-bottom: none; }
.use-first-name { flex: 1; font-weight: 600; color: #222; }
.use-first-qty  { color: #888; font-size: 13px; min-width: 70px; text-align: right; }
.use-first-date { font-size: 12px; min-width: 130px; text-align: right; }
.expiry-warning  { color: #E65100; }
.expiry-critical { color: #C62828; font-weight: 600; }
.expiry-ok       { color: #2D7A3E; }
