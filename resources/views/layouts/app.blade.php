        ...
        <aside class="sidebar">
            <nav class="sidebar-nav">
                ...
                <a href="{{ route('storage.index') }}" class="nav-item {{ request()->routeIs('storage.*') ? 'active' : '' }}">
                    Storage
                    @php
                        $storageAlertCount = \App\Models\StorageItem::where('user_id', auth()->id())
                            ->where('status', 'active')
                            ->whereNotNull('expiration_date')
                            ->where('quantity', '>', 0)
                            ->where('expiration_date', '<=', today()->addDays(3))
                            ->count();
                    @endphp
                    @if($storageAlertCount > 0)
                        <span class="nav-badge">{{ $storageAlertCount }}</span>
                    @endif
                </a>
